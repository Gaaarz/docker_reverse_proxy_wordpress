...
[ ! -f $dirpath/reverse-proxy/nginx-conf/nginx.conf-sample ] \
    && echo "Sample NGINX config is not found" && exit 1
cat $dirpath/reverse-proxy/nginx-conf/nginx.conf-sample | \
    sed "s/ROOT_DOMAIN/$domain/" > $dirpath/reverse-proxy/nginx-conf/nginx.conf

if command -v docker-compose &> /dev/null; then
    docker-compose -f $dirpath/reverse-proxy/docker-compose.yml up -d
else
...
docker-compose -f $dirpath/reverse-proxy/docker-compose.yml restart nginx
...
docker-compose -f $dirpath/reverse-proxy/docker-compose.yml run --rm --entrypoint "certbot certonly --webroot \
